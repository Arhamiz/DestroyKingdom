...
internal static class KingdomAnnexationAction
{
    ...
    public static void Apply(Kingdom annexedKingdom, Kingdom annexingKingdom, bool showNotification = true)
    {
        ...
        TextObject tt1 = new TextObject("{=DK_067}{ANEXXING} annexed {ANNEXED}. {NAME} and {COUNT} out of {SOURCE} vassal clans decided to collaborate.");
        tt1.SetTextVariable("ANNEXXING", annexingKingdom.ToString());
        tt1.SetTextVariable("ANNEXED", annexedKingdom.ToString());
        tt1.SetTextVariable("NAME", oldRulerName.ToString());
        tt1.SetTextVariable("COUNT", collaboratingClansCount.ToString());
        tt1.SetTextVariable("SOURCE", vassalClans.Count.ToString());
        if (showNotification)
        {
            MBInformationManager.AddQuickInformation(tt1);
